...
import { createGameSession, generateCoinFlip, saveGameRecord, type GameSession } from '@/lib/provablyFair';
...
export default function PvPCoinFlip() {
  ...
  const handleCreatePrivateRoom = () => {
    if (selectedStakeForPrivate > gameBalance) {
      toast.error(`Not enough funds! You need ${formatUSDT(selectedStakeForPrivate)} but only have ${formatUSDT(gameBalance)}. Transfer more from your main wallet.`);
      return;
    }
  ...
  const startCoinFlip = async (room: Room) => {
    ...
    const winnerSide: CoinSide = await generateCoinFlip(
      session.serverSeed,
      session.clientSeed,
      session.nonce
    );
  ...
  const showResult = (room: Room, winnerSide: CoinSide) => {
    ...
    if (won) {
      ...
      recordGamePlayed(username || 'guest', room.stakeAmount, winAmount, true);
    } else {
      setStats(prev => ({
        wins: prev.wins,
      ...
      recordGamePlayed(username || 'guest', room.stakeAmount, 0, false);
    }
  ...
  const handleRematch = () => {
    ...
    if (currentRoom.stakeAmount > gameBalance) {
      toast.error(`Not enough funds for rematch! You need ${formatUSDT(currentRoom.stakeAmount)} but only have ${formatUSDT(gameBalance)}. Transfer more from your main wallet.`);
      return;
    }
