...
import { createGameSession, generateCrashMultiplier, saveGameRecord, type GameSession } from '@/lib/provablyFair';
...
export function CrashGame() {
  ...
  const [events, setEvents] = useState<GameEvent[]>([
    { id: '1', player: username || 'Guest', type: 'join', amount: 50, time: '12:34:21' },
    { id: '2', player: 'CryptoTrader', type: 'join', amount: 100, time: '12:34:22' },
    { id: '3', player: 'MoonShot', type: 'join', amount: 25, time: '12:34:23' },
    { id: '4', player: 'DiamondHands', type: 'cashout', multiplier: 2.45, amount: 50, time: '12:34:28' },
  ]);
  ...
  useEffect(() => {
    ...
    if (gameState === 'waiting') {
      ...
      (async () => {
        ...
        crashAt = await generateCrashMultiplier(
          session.serverSeed,
          session.clientSeed,
          session.nonce
        );
      ...
      interval = setInterval(() => {
        ...
        if (newMultiplier >= crashAt) {
          ...
          if (hasPlacedBet && !hasCashedOut) {
            ...
            recordGamePlayed(username || 'guest', betAmount, 0, false);
          }
  ...
  const handlePlaceBet = () => {
    ...
    setPlayers(prev => [...prev, { id: 'player', name: username || 'You', bet: betAmount, status: 'active' }]);
  ...
  const handleCashout = () => {
    ...
    recordGamePlayed(username || 'guest', betAmount, payout, true);
